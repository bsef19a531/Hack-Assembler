...
{
    ifstream del_file;
    del_file.open("machinecode_t1.hack");
    if(!del_file){}
    else
    {
        del_file.close();
        remove("machinecode_t1.hack");
    }
}
...
{
    fstream write_file;
	write_file.open("machinecode_t1.hack", ios::out | ios::app);
	if (!write_file) 
    {
...
{
    fstream read_file;
    read_file.open("machinecode_t1.asm", ios :: in);
    if (!read_file)
    {
